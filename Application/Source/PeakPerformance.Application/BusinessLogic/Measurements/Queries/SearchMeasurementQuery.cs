...
public class SearchMeasurementQuery(MeasurementSearchOptions options) : IRequest<ResponseWrapper<PagingResult<MeasurementDto>>>
{
    public MeasurementSearchOptions Options { get; set; } = options;

    public class SearchMeasurementQueryHandler(IDatabaseContext db, IIdentityUser identityUser, IMapper mapper) : IRequestHandler<SearchMeasurementQuery, ResponseWrapper<PagingResult<MeasurementDto>>>
    {
        public async Task<ResponseWrapper<PagingResult<MeasurementDto>>> Handle(SearchMeasurementQuery request, CancellationToken cancellationToken)
        {
            var options = request.Options;

            var userId = options.UserId ?? identityUser.Id;

            var predicates = new List<Expression<Func<Measurement, bool>>>();
